...
const testimonials = [
    ...
    },
    {
        ...
        quote: 'Ut enim ad minima veniam, quis nostrum exercitationem ullam corporis suscipi tnisi ut aliquid commodi.',
        stars: 4.5,
    },
    {
        name: 'Maria Kate',
    ...
    return (
        <>
            ...
            {[...Array(5 - Math.ceil(stars))].map((_, i) => (
                <span key={i + fullStars} className="star">&#9734;</span>
            ))}
        </>
    );
};
                ...
                {Array.from({ length: 3 }).map((_, i) => (
                    <div className="testimonial" key={i}>
                        <div className="testimonial-img">
                            <img src={testimonials[getTestimonialIndex(i)].image} alt={testimonials[getTestimonialIndex(i)].name} />
                        </div>
                        <h5>{testimonials[getTestimonialIndex(i)].name}</h5>
                        <h6>{testimonials[getTestimonialIndex(i)].title}</h6><br />
                        <p>{testimonials[getTestimonialIndex(i)].quote}</p>
                        <div className="stars">{generateStars(testimonials[getTestimonialIndex(i)].stars)}</div>
                    </div>
                ))}
            </div>
